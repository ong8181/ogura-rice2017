####
#### Insect data, dimension reduction
####
...
ps_d <- phyloseq(sample_data(d_nona[,c("date", "n_week", "treatment", "sampling_point", "sample_code")]),
                  otu_table(d_nona[,7:9], taxa_are_rows = FALSE))
# Remove "0" samples
...
g2 <- g_df %>% ggplot(aes(x = NMDS1, y = NMDS2)) +
  ...
  ggtitle("Insect abundance, NMDS, Bray-Curtis")
...
saveRDS(list(g1, g2), "../07_FormatFigs/data_robj/Insect_DimRed.obj")
