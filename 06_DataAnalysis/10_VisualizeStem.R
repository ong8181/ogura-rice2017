...
mdr_stm <- readRDS("07_CompileMDRresOut/mdr_stm.obj")
mdr_stm_long <- readRDS("07_CompileMDRresOut/mdr_stm_long.obj")

# Extract causal variables
## N of stems
cause_vars1 <- mdr_stm %>% select(starts_with("smapc_")) %>% colnames %>% str_split(pattern = "smapc_from_") %>% sapply(`[`, 2)
...
# Specify data set
mdr_res_used <- mdr_stm
mdr_res_long_used <- mdr_stm_long
...
g1 <- mdr_res_tax %>% filter(!is.na(smapc)) %>% 
  ...
  ggtitle("Influence on N of stems\nper unit variable") +
  theme_bw() +
  NULL
...
ggsave(sprintf("%s/StemDiff_overall.pdf", outdir), g_all, width = 14, height = 9)
saveRDS(g_all, "../07_FormatFigs/data_robj/Smap_Nstem-all.obj")
saveRDS(list(g1, g2, g3), "../07_FormatFigs/data_robj/Smap_Nstem.obj")
