...
mdr_spd <- readRDS("07_CompileMDRresOut/mdr_spd.obj")
mdr_spd_long <- readRDS("07_CompileMDRresOut/mdr_spd_long.obj")

# Extract causal variables
## Height
cause_vars1 <- mdr_spd %>% select(starts_with("smapc_")) %>% colnames %>% str_split(pattern = "smapc_from_") %>% sapply(`[`, 2)
...
# Specify data set
mdr_res_used <- mdr_spd
mdr_res_long_used <- mdr_spd_long
...
ggsave(sprintf("%s/SPADDiff_overall.pdf", outdir), g_all, width = 14, height = 9)
saveRDS(g_all, "../07_FormatFigs/data_robj/Smap_SPAD-all.obj")
saveRDS(list(g1, g2, g3), "../07_FormatFigs/data_robj/Smap_SPAD.obj")
